...
#from PFNet_Dense import PFNet
from CSRNet_deconv import PFNet
...
parser = argparse.ArgumentParser(description='PyTorch PFNet-deconv')
...
def main():
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        prec1, prec_mse = validate(val_list, model, criterion)
    
        is_best = prec1 < best_prec1
        best_mse = min(prec_mse, best_mse)
        best_prec1 = min(prec1, best_prec1)
        
        line = ' * best MAE {mae:.3f} * best MSE {mse:.3f}'.format(mae=best_prec1, mse=best_mse)
...
def train(train_list, model, criterion, optimizer, epoch):
    ...
    for i,(img, target)in enumerate(train_loader):
        ...
        #print('#### Aftr Image size: {}'.format(img.shape))
        #print('#### Aftr Target size: {}'.format(target.shape))
        img = img.cuda()
        img = Variable(img)
        #print('***the train input image resolution is: {}***'.format(img.size()))
        output = model(img)
        #print('#### Output size: {}'.format(output.shape))        
        #print('***the train output image resolution is: {}***'.format(output.size()))
...
def validate(val_list, model, criterion):
    print ('begin test')
    val_trans_list = [transforms.ToTensor(),transforms.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225])]
    test_loader = torch.utils.data.DataLoader(
    dataset.listDataset(val_list,
                   shuffle=False,
                   transform_list=val_trans_list,
    ...
    for i,(img, target) in enumerate(test_loader):
        img = img.cuda()
        img = Variable(img)
        #print('===the val input image resolution is: {}==='.format(img.size()))
        output = model(img)
        #print('===the val output image resolution is: {}==='.format(output.size()))
    ...
    mse = np.sqrt(mse.cpu()/len(test_loader))
    line1 = ' * MAE {mae:.3f} '.format(mae=mae)
    line2 = ' * MSE {mse:.3f} '.format(mse=mse)
    with open('logs/{}_{}.log'.format(time_stp, args.arch), 'a+') as flog:
        print(line1)
        print(line2)
        flog.write('{}\n'.format(line1))
